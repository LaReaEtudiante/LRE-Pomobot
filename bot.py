...
async def leaderboard(ctx):
    ...
    if not top10:
        return await ctx.send("Aucun utilisateur pour l'instant.")
    desc = "\n".join(f"#{i+1} <@{uid}> : {mins} min" for i,(uid,mins) in enumerate(top10))
    embed = discord.Embed(title="Leaderboard Pomodoro", color=MsgColors.PURPLE.value, description=desc)
    await ctx.send(embed=embed)

@bot.command(name='clear_stats', help='Vider les données (admin)')
@is_admin()
async def clear_stats(ctx):
    db = TinyDB('leaderboard.json')
    db.purge()
    await ctx.send("Toutes les statistiques ont été supprimées.")
...
async def help_cmd(ctx):
    ...
    embed.add_field(name="Administrateur", value=
        ...
        "`clear_stats` – vider stats", inline=False)
    await ctx.send(embed=embed)
